                  ...
                  <div className="overflow-x-auto">
                    <table id="tabla-productos" className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {products.length === 0 ? (
                          <tr>
                            <td colSpan={4} className="py-8 text-center text-slate-400 text-xs font-bold">No hay productos en el catálogo de Firestore.</td>
                          </tr>
                        ) : (
                          products.map(p => {
                            const totalStock = p.variantes.reduce((acc, v) => acc + v.stock, 0);
            ...
            <div id="seccion-pedidos-content" className="space-y-6 animate-fade-in">
              ...
              <div id="encabezado-pedidos" className="bg-white p-6 rounded-2xl border border-slate-150/40 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <button
                  ...
                  onClick={handleRefreshData}
                  className="px-4 py-2 bg-slate-50 hover:bg-slate-100 border border-slate-200/50 rounded-xl text-xs font-bold text-slate-700 flex items-center justify-center gap-2 transition-all cursor-pointer self-start sm:self-auto shrink-0"
                >
                  <RefreshCw className="w-3.5 h-3.5" />
                  Actualizar Pedidos
                </button>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 text-xs text-slate-700">
                      {orders.length === 0 ? (
                        <tr>
                          <td colSpan={5} className="py-8 text-center text-slate-400 font-bold">No hay transacciones registradas en Firestore todavía.</td>
                        </tr>
                      ) : (
                        orders.map(o => (
                          <tr key={o.id} className="hover:bg-slate-50/30 transition-colors">
                            <td className="py-4">
                              <p className="font-mono font-black text-slate-900 text-xs">ID: {o.id.slice(0, 8)}...</p>
