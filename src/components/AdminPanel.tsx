...
export default function AdminPanel({
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setProductForm(prev => ({
          ...prev,
          imagenes: [...prev.imagenes.filter(img => img !== ''), reader.result as string]
        }));
  ...
  return (
    <div id="admin-dashboard-container" className="bg-[#FAFBFD] min-h-screen font-sans text-slate-800 flex relative overflow-x-hidden">
      ...
      <aside 
        id="admin-sidebar"
        className={`fixed top-0 bottom-0 left-0 w-64 bg-slate-950 text-slate-200 flex flex-col py-8 px-6 z-40 transition-transform duration-300 border-r border-slate-900 lg:translate-x-0 overflow-y-auto ${
                ...
                <div id="formulario-producto-card" className="xl:col-span-1 bg-white p-6 rounded-2xl border border-slate-150/40 shadow-sm space-y-6">
                  ...
                  <form onSubmit={handleSaveProduct} className="space-y-4">
                    ...
                    <div className="space-y-2 border-t border-slate-100 pt-3">
                      <label className="text-[10px] uppercase font-bold text-slate-400 block mb-1">Imágenes del Producto</label>
                      
                      {/* Grid de imágenes */}
                      <div className="grid grid-cols-2 gap-2">
                        {productForm.imagenes.filter(img => img !== '').map((img, idx) => (
                          <div key={idx} className="relative aspect-video rounded-xl overflow-hidden border border-slate-200 bg-slate-50">
                            <img src={img} alt={`Imagen ${idx + 1}`} referrerPolicy="no-referrer" className="w-full h-full object-cover" />
                            <button
                              type="button"
                              onClick={() => setProductForm(prev => ({ ...prev, imagenes: prev.imagenes.filter((_, i) => i !== idx) }))}
                              className="absolute top-1 right-1 bg-slate-900/80 text-white p-1 rounded-lg hover:bg-slate-950 transition-colors cursor-pointer"
                            >
                              <X className="w-3 h-3" />
                            </button>
                          </div>
                      ...
                      <div className="flex gap-2 mt-2">
                        <input
                          ...
                          className="flex-1 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-[10px] focus:ring-1 focus:ring-slate-300"
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                              e.preventDefault();
                              const val = e.currentTarget.value;
                              if(val) {
                                setProductForm(prev => ({...prev, imagenes: [...prev.imagenes, val]}));
                                e.currentTarget.value = '';
                              }
                            }
                          }}
                        />
                        <label className="flex items-center justify-center p-2 bg-slate-900 text-white rounded-lg cursor-pointer hover:bg-slate-800 transition-colors">
